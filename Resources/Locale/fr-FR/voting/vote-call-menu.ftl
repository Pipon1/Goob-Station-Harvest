...
ui-vote-type-restart = Redémarrer la manche
ui-vote-type-gamemode = Mode de jeu suivant
ui-vote-type-map = Carte suivante
...
ui-vote-follow-button = Suivre le joueur

# Timeout text if a standard vote type is currently on timeout.
ui-vote-type-timeout = Ce type de vote a été lancé trop récemment ({$remaining})

# Unavailable text if a vote type has been disabled manually.
ui-vote-type-not-available = Ce type de vote est désactivé

# Vote option only available for specific users.
ui-vote-trusted-users-notice =
  Cette option de vote est uniquement disponible pour les joueurs ayant suffisamment de temps de jeu ou figurant sur la liste blanche.
  De plus, vous devez être un fantôme depuis { $timeReq } secondes.

# Warning to not abuse a specific vote option.
ui-vote-abuse-warning =
  Attention !
  Tout abus du système de vote d'expulsion peut entraîner un bannissement définitif !
...
cmd-votemenu-help = Utilisation : {$command}
