# All spelling mistakes and broken english are intentional!
# I hate saving paper contents in ftl files

## Headers and reusable elements

-delivery-header-nanotrasen = [color=blue]
                                                                      ╔══════════════════╗
                                                                      ║███░███░░░░██░░░░░║
                                                                      ║░██░████░░░██░░░░░║
                                                                      ║░░█░██░██░░██░█░░░║
                                                                      ║░░░░██░░██░██░██░░║
                                                                      ║░░░░██░░░████░███░║
                                                                      ╚══════════════════╝[/color]

-delivery-header-nanotrasen-alternate-timeline = [color=red]
                                                                ╔══════════════════╗
                                                                ║███░███░░░░██░░░░░║
                                                                ║░██░████░░░██░░░░░║
                                                                ║░░█░██░██░░██░█░░░║
                                                                ║░░░░██░░██░██░██░░║
                                                                ║░░░░██░░░████░███░║
                                                                ╚══════════════════╝[/color]

-delivery-header-syndicate = [color=#ff0000]
                                                                          ╔══════════════════╗
                                                                          ║░░░░░████████░░░░░║
                                                                          ║░░░░░██░░░░░░░░░░░║
                                                                          ║░░░░░████████░░░░░║
                                                                          ║░░░░░░░░░░░██░░░░░║
                                                                          ║░░░░░████████░░░░░║
                                                                          ╚══════════════════╝[/color]

## Spam letters

delivery-spam-robust-toolboxes =   Offre spéciale ! Boîtes à outils robustes !
    .desc =         Une publicité pour les boîtes à outils robustes.
    .content =      [color=blue][head=1]
                        ░░▄▀░░
                        ░▄█▄▄▀ [head=3]ROBUST - BOÎTES À OUTILS ET OUTILS[/head]
                        ██▀░░░ [/head][/color]

                        {"[bold]ACHETEZ UNE BOÎTE À OUTILS, OBTENEZ UN ENSEMBLE D'OUTILS GRATUIT ![/bold]"}

                        COMME VOUS ÊTES L'UN DE NOS CLIENTS APPRÉCIÉS, VOUS OBTENEZ UN BONUS CLIENT, VOS OUTILS SONT SANS ROUILLE ET SANS PLOMB !!! N'EST-CE PAS INCROYABLE ? LA BOÎTE À OUTILS, EN REVANCHE, VIENT AVEC DU PLOMB SUPPLÉMENTAIRE ! INCROYABLE POUR EXPLOSER DES TÊTES ET ARRÊTER LES RADIATIONS !

                        {"[bold]TOUT CELA ET POSSIBLEMENT PLUS POUR UN SEUL ORGANE ![/bold]"}

                        ROBUST - BOÎTES À OUTILS ET OUTILS :
                        -SANS PLOMB ET SANS AMIANTE !
                        -OU AVEC PLOMB ET AMIANTE, SI VOUS PRÉFÉREZ !
                        -BON MARCHÉ ! UN SEUL ORGANE ! C'EST MOINS QUE DEUX ORGANES !
                        -N'A PAS BESOIN D'ÊTRE VOTRE ORGANE ! ON NE JUGE PAS !

delivery-spam-reasons-to-chose-nanotrasen =   Raisons de choisir Nanotrasen !
    .desc =         Une publicité pour Nanotrasen.
    .content =      {-delivery-header-nanotrasen}

                        {"[head=2]LES TROIS MEILLEURES RAISONS POUR LESQUELLES LE SYNDICAT EST INCOMPÉTENT[/head]"}

                        {"[bold]NUMÉRO UN[/bold]"}
                        LEURS AGENTS DORMANTS SONT INCOMPÉTENTS ! ILS NE PEUVENT MÊME PAS TUER UN PASSAGER QUI A UNE ENVIE DE MOURIR !

                        {"[bold]NUMÉRO DEUX[/bold]"}
                        LEURS CIVILS SONT FAIBLES CONTRE LES BALLES ! CROYEZ-MOI, NOUS AVONS ESSAYÉ ! CONTRAIREMENT AUX CIVILS DE NANOTRASEN, LES CIVILS DU SYNDICAT MEURENT D'UNE BALLE À LA TÊTE ! DES BALLES QUE NOUS AVONS !

                        {"[bold]NUMÉRO TROIS[/bold]"}
                        LEUR LOGO EST HORRIBLE ! ILS PENSENT QU'ILS SONT COOL AVEC LEUR LOGO ! OOH, REGARDEZ-MOI, JE SUIS TELLEMENT COOL ! OOH, UN SERPENT QUI EST AUSSI UN S ! COMME C'EST CRÉATIF ! MON FILS DE TROIS ANS POURRAIT DESSINER UN MEILLEUR LOGO !

delivery-spam-reasons-to-choose-syndicate =   Raisons de choisir le Syndicat !
    .desc =         Une publicité pour le Syndicat.
    .content =      {-delivery-header-syndicate}

                        {"[head=2]LES TROIS MEILLEURES RAISONS POUR LESQUELLES NANOTRASEN EST INCOMPÉTENT[/head]"}

                        {"[bold]NUMÉRO UN[/bold]"}
                        LEURS PISTOLETS SONT NULS ! ILS N'ONT MÊME PAS DE FUSILS DE PRÉCISION ! LEURS FORCES DE SÉCURITÉ NE PEUVENT MÊME PAS PORTER DE GROS PISTOLETS DANS LA PLUPART DES SITUATIONS !

                        {"[bold]NUMÉRO DEUX[/bold]"}
                        LEURS COMMANDANTS ? ILS MEURENT D'UN SEUL COUP ! PAS D'ARMURE COOL ! JUSTE BANG, MORT ! NUL ! NOS COMMANDANTS ONT DES COMBINAISONS BLINDÉES COOL !

                        {"[bold]NUMÉRO TROIS[/bold]"}
                        LEURS MÉTHODES DE MEURTRE SONT SANS INSPIRATION ! C'EST JUSTE DES PISTOLETS ! IL N'Y A PAS DE JET DE GENS DANS L'ESPACE PROFOND, PAS DE NOURRITURE POUR LES RECYCLEURS AVEC LE MODE SÉCURITÉ DÉSACTIVÉ, PAS DE BOMBE GLISSÉE DANS LES POCHES ! TELLEMENT ENNUYEUX !

delivery-spam-tired-of-science =   Fatigué que la science explose tout le temps ?
    .desc =         Suivez ces étapes simples pour vous assurer que ça n'arrive plus jamais !
    .content =      [head=3]La science vous ADORERA pour ça !!

                        ÊTES-[bold]VOUS[/bold] FATIGUÉ que le département scientifique de votre station explose sans faire de véritable science ?
                        Eh bien, vous avez de la chance ![/head]

                        Suivez ce guide simple, et nous nous assurerons que votre science [italic]ne fonctionne plus jamais ![/italic]

                        Faites simplement ce qui suit :
                        - Étape un : Localisez le serveur de recherche de votre département scientifique
                        - Étape deux : Désancrez le serveur de recherche du sol
                        - Étape trois : Lancez le serveur de recherche dans l'espace, de préférence en direction du dojo secret de l'espace du clan des araignées
                        - Étape quatre : Attendez environ 3-5 services de travail
                        - Étape cinq : Nos travailleurs chez Spid-ex Inc vous fourniront un (1) disque technologique par semaine.

                        {"[color=lightgray]Remarque : Le clan des araignées n'est pas responsable des punitions émises par vos supérieurs.[/color]"}

delivery-spam-free-all-access =   ALL AXCÈS GRATUIT !!
    .desc =         Tu as déjà voulu avoir un all acces gratuit ?!
    .content =      [head=3]Tu as déjà voulu avoir un [italic][color=green][bold]all acces gratuit !?!?[/bold][/color][/italic][/head]
                        {"[head=2]Eh bien, MAINTENANT TU PEUX !![/head]!"}

                        Tout ce que tu as à faire est d'appeler [color=blue]555-GOUR-LECKSSS[/color] et déclare ton numéro d'identification de station !!! 
                        Une fois que tu l'as fait, nous pouvons simplement interroger à distance le portefeuille du département cargo de votre station, en extractant nos frais requis de trois paiements [italic] FACILES[/italic], ce qui te permet de réclamer ton
                        {"[head=2][color=green] [bolditalic] AA GRATUIT !!!!!!!!!![/bolditalic][/color][/head]"}


                        {"[color=gray]"}
                        {"[bullet/]Remarque : l'identifiant de station doit être énoncé au format \"NT/NX - ###\""}
{"[bullet/]Remarque : les paiements versés au département cargo de la station du client s'élèvent à environ 5 000 spesos par transaction, non compris les frais de traitement individuels"}
                    {"[bullet/]Remarque : Nous chez Gour-Lecksss LMT. ne sommes pas responsables si le HoP de votre station vous force à remplir un formulaire AA RÉEL s'il apprend l'existence de cette lettre"}
                    {"[/color]"}

delivery-spam-centcomm-retribution =   AVIS DE NANOTRASN !!
.desc =         Un avis officiel du PDG de Nanotrasn ?!
.content =      [color=red] CECI EST UN AVIS OFICEL DU CHEF DE [color=blue]NANOTRASN[/color][/color]

                    Madame, Monsieur, ou autre personnell de station insignificant

                    Si vous ne voulez pas que cette station soit déclarée comme Non rentable aux yeux de
                    {"[head=2][italic] Notre Grand et Glorieux [color=blue]Nanotransen[/color][/head]"}
                    Alors vous devez organiser pour que trois [color=blue]Nt[/color] Piles standard de [color=blue]nt[/color] Lingots d'or standard soient envoyés aux Postes Commerciaux Éloignés de votre station dans les 5 [color=blue]nT[/color] services de travail standard.

                    {"[head=2][color=red]IGNORET CET ORDRE AU RISQUE D'UNE RÉTRIBUTION DE [color=green]CENTCO[/color] !!!!![/head][/color]"}

delivery-spam-alternate-timeline =   Envoyez des renforts !
.desc =         Un avis officiel d'... une timeline alternative ?
.content =      {-delivery-header-nanotrasen-alternate-timeline}
                    {"[head=2]Ceci est un avis officiel du [color=red]Officier de sécurité en chef[/color] à la station spatiale 15 de Nanotrasen.[/head]"}

                    À celui qui reçoit cette lettre. Je suis le sergent Rigel. Mon occupation est le CdS. Nous avons besoin d'assistance immédiate.

                    Notre station est actuellement attaquée par des Agents Atomiques, cette lettre est jetée dans une anomalie bluespace déstabilisée créée par notre [color=purple]Chef de recherche[/color].

                    Je suis actuellement barricadé sur le pont, si vous recevez ce message, veuillez envoyer de l'aide immédiatement. Je ne sais pas combien de temps nous pouvons encore tenir.

                    Gloire à Nanotrasen.

delivery-spam-narsie-cult =   Les enfants de Nar'Sie
.desc =         Un culte local recrute des membres.
.content =      [color=#134975][head=2]Les enfants de Nar'Sie[/head][/color]
                                                    Le début d'une nouvelle ère
                    {"[bold]══──══──══──══──══──══──══──══──══──══──══[/bold]"}

                    {"[head=3]Vous sentez-vous perdus dans l'immensité de notre cosmos ?[/head]"}
                    À l'ère moderne, il est facile pour les âmes égarées de se sentir comme des rouages dans la machine des grandes corporations.

                    {"[head=3]Vous avez l'impression d'être fait pour un meilleur objectif ?[/head]"}
                    Êtes-vous fatigué de la vie de médiocrité qui vous est imposée ? Nettoyer les sols, livrer des boîtes ou remplir des papiers sans fin ?

                    {"[head=3]Voulez-vous rendre la galaxie meilleure ?[/head]"}


                    Si vous avez répondu « oui » à l'une de ces questions, contactez l'un de nos représentants dès aujourd'hui ! Nous avons des membres dans des stations de toute la galaxie qui sont impatients d'accueillir de nouveaux membres dans notre troupeau. Soyez l'une des lames qui aide à accueillir le géomètre du sang dans notre univers afin que tous connaissent sa béatitude !

                    Tout ce que vous avez à faire est de dire [color=#FF0000][italic]« Sas'so c'arta forbici ! »[/italic][/color]

delivery-spam-rage-cage =   Tu veux te battre ?!
.desc =         Publicité pour un club de combat local.
.content =      [color=#aaaaaa]▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀[/color]
                                                {"[bold][head=1]LA RAGE CAGE[/head][/bold]"}
                    {"[color=#aaaaaa]▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬[/color]"}
                    {"[bold][color=#FF0000][head=3]VOULEZ-VOUS VOUS BATTRE ?[/head][/color][/bold]"}
                    {"[bold][color=#FF0000][head=3]VOULEZ-VOUS GAGNER ?[/head][/color][/bold]"}
                    {"[bold][color=#FF0000][head=3]VOULEZ-VOUS DOMINER ?![/head][/color][/bold]"}



                    Alors venez à... [color=#FF0000][bold][head=2]LA RAGE CAGE[/head][/bold][/color]



                    Caché dans les profondeurs de votre station Nanotrasen locale se trouve la [color=#FF0000][bold]RAGE CAGE[/bold][/color] brevetée. Une arène de combat électrifiée conçue pour les combattants les plus forts, la [color=#FF0000][bold]RAGE CAGE[/bold][/color] sépare les perdants des gagnants, les bleus des soldats, et les losers des champions.
                    ──────────────────────────────────────────
                    Dans la [color=#FF0000][bold]RAGE CAGE[/bold][/color], il n'y a qu'une seule règle : [italic]Deux combattants entrent. Un combattant sort. [/italic]
                    ──────────────────────────────────────────
                    Pas d'armes, pas d'armure, juste du pur [bold]COMBAT[/bold]. Ne manquez pas l'occasion en étant une [bold]MAUVIETTE[/bold]. Remportez la gloire d'être le combattant le plus robuste de votre station dans la [color=#FF0000][bold]RAGE CAGE[/bold][/color] aujourd'hui !

delivery-spam-evil-lizard =  N'OUVREZ PAS CE COURRIER
.desc =         Vous avez été maudit !
.content =      [color=#FF0000][bold][head=2]ARRÊTEZ[/head][/bold][/color]

                    Si vous lisez cette lettre...[color=#FF0000][head=3]VOUS ETE DEJA MAUDIT !!![/head][/color]

                    Je suis désolé de faire ça, mais je dois vous avertir sur :

                            {"[color=#FF0000][head=1]Le Fantôme du Lézardd Sanglant[/head][/color]"}

                    Tout a commencé quand j'ai reçu une lettre par la poste : c'était une image effrayante d'une peluche de lézard avec des YEUX SANGLANTS qui me fixaient DROIT DANS LES YEUX. La lettre disait que j'étais maudit...et si je n'envoyais pas cette lettre à 30 personnes dans les 30 jours, le lézard sanglant viendrait au milieu de la nuit...

                    {"[italic]et ME TUERAIT.[/italic]"}

                    Je suis désolé......mais tu ai l'une des 30 personnes à qui je doi envoyer ça...et maintenant tu ai maudit aussi...

                    Sil te plait, envoye cette lettre à 30 autres personnes pour arrêter la malédicion ! Vous pouvez encore vous sauver ! Y a encore du temps ! Ne laisse pas le lézard sanglant te tuer aussi, et prenez ça ! Ça va te protégé des esprits sombres...[head=3]POUR L'INSTANT[/head]




                    {"[head=1]OH NON LE VOILÀ !!!!!!!![/head]"}

                    ░░░░░░░░░█░░[color=#67CC40]████████[/color]█[color=#67CC40]███[/color]░░░░░░░░░░
                    ░░░░░░[color=#FF0000]████[/color]█[color=#6EC543]█[/color][color=#67CC40]███████[/color]█[color=#FF0000]██████[/color]░░░░░░░░
                    ░░░░[color=#FF0000]████[/color][color=#6EC543]██[/color][color=#67CC40]████████[/color][color=#FF0000]██████[/color][color=#FFFFFF]██[/color][color=#FF0000]█[/color][color=#B53737]█[/color]░░░░░░
                    ░░░░[color=#FF0000]██[/color][color=#6EC543]██[/color][color=#67CC40]██████████[color=#FF0000]████████[/color][color=#B53737]██[/color]░░░░░░
                    ░░[color=#6EC543]██████[/color][color=#67CC40]██████████[/color][color=#FF0000]███████[/color][color=#B53737]███[/color]░░░░░░
                    {"[color=#6EC543]██████[/color][color=#67CC40]██████████████[/color][color=#FF0000]███[/color][color=#B53737]███[/color]░░░░░░░░"}
                    {"[color=#6EC543]██████[/color][color=#67CC40]██████████████[/color][color=#FF0000]█[/color][color=#6EC543]█████[/color]░░░░░░░░"}
                    {"[color=#6EC543]██████████[/color][color=#67CC40]██████████[/color][color=#FF0000]█[/color][color=#67CC40]███[/color]░░░░░░░░░░"}
                    ░░[color=#6EC543]██[/color][color=#FF2020]██[/color][color=#FF3D3D]██[/color][color=#6EC543]████████[/color][color=#67CC40]████[/color][color=#86E158]██[/color]░░░░░░░░░░░░
                    ░░░░[color=#FF2020]██[/color][color=#FF3D3D]██[/color]░░░░[color=#86E158]████████[/color][color=#6EC543]██[/color][color=#61D034]██[/color]░░░░░░░░[color=#56B037]██[/color]
                    ░░░░[color=#FF2020]██[/color]░░░░[color=#A8EB7A]██[/color][color=#B5EE85]██████[/color][color=#A8EB7A]██[/color][color=#6EC543]████[/color][color=#61D034]██[/color]░░░░[color=#56B037]██[/color][color=#48A926]██[/color]
                    ░░░░░░░░[color=#A8EB7A]██[/color][color=#B5EE85]██████████[/color][color=#A8EB7A]██[/color][color=#6EC543]████[/color][color=#56B037]██[/color][color=#48A926]██████[/color]
                    ░░░░[color=#6EC543]██[/color][color=#61D034]██[/color][color=#A8EB7A]██[/color][color=#B5EE85]██████████[/color][color=#A8EB7A]██[/color][color=#48A926]████████████[/color]
                    ░░░░[color=#6EC543]████[/color][color=#86E158]██[/color][color=#A8EB7A]██[/color][color=#B5EE85]████[/color][color=#A8EB7A]██[/color][color=#86E158]██[/color][color=#61D034]████[/color][color=#6EC543]██[/color][color=#48A926]████[/color][color=#52A037]██[/color]░░
                    ░░░░[color=#6EC543]████[/color][color=#61D034]██[/color][color=#86E158]████████[/color][color=#61D034]██[/color][color=#6EC543]██████[/color][color=#52A037]████[/color]░░░░
                    ░░░░░░[color=#6EC543]████[/color]░░░░░░░░[color=#6EC543]████████[/color]░░░░░░░░
                    ░░░░░░░░░░░░░░░░░░░░[color=#6EC543]████[/color]░░░░░░░░░░

delivery-spam-parents-need-money =  Aide maman et papa !
.desc =         Des parents ont besoin d'aide financière.
.content =      [bold]Bonjour mon enfant,[/bold]
                    C'est tes parents qui écrivent : nous avons besoin d'argent ! Nos impôts ont été inspectés et nous devons 100 000 spesos au gouvernement ! S'il te plaît aide-nous ! Ils veulent juste des cartes-cadeaux, tu devras donc nous envoyer 100 cartes-cadeaux Bisa de 1 000 spesos.

                    Veuillez envoyer à :
                    50192 Spess Lane
                    Station City, Ignius 40195-243
                    Quadrant gamma
                    Système Guilimin

                    Si nous ne recevons pas cet argent dans 10 jours, le gouvernement viendra prendre notre maison d'enfance et nous serons sans abri.

                    {"[bold]Merci et nous t'aimons,[/bold]"}
                    {"[italic]parents[/italic]"}

delivery-spam-voyage-advertisement =  Rejoignez-nous pour le voyage inaugural !
.desc =         Publicité pour un voyage relaxant.
.content =      [head=2]Rejoignez-nous pour le voyage inaugural du...[/head]

                        ░█▀▀░█▀▀░░░█▀▄░█▀▀░█░░░█▀█░█░█░█▀█░█▀▄
                        ░▀▀█░▀▀█░░░█▀▄░█▀▀░█░░░█▀█░▄▀▄░█░█░█▀▄
                        ░▀▀▀░▀▀▀░░░▀░▀░▀▀▀░▀▀▀░▀░▀░▀░▀░▀▀▀░▀░▀


                    {"[bold]══════════════════════════════════════════[/bold]"}

                    Le dernier en Comfortech™ et les plus beaux paysages de ce côté du système d'Iraxsi ! Le [italic]SS Relaxor[/italic] est un croiseur de luxe ultra-moderne qui vous emmène dans le voyage d'une vie !

                    {"[head=3]Découvrez les lacs de phosphore de Galimar* depuis le confort de nos LuxuCabins™ avec toutes les commodités modernes que vous pourriez souhaiter ![/head]"}

                    {"[head=3]Regardez avec émerveillement l'œil du cosmos** tout en dégustant des repas de nos chefs de classe galactique cinq étoiles ![/head]"}

                    {"[head=3]Explorez les ruines d'Agathar***, maintenant ouvertes au public avec l'aide des meilleurs scientifiques de Nanotrasen. Tous les mystères de l'ancienne civilisation agathéienne sont fouillés et affichés pour votre plaisir visuel ![/head]"}

                    Pour le coût faible et bas de 5 000 spesos par nuit, la croisière de luxe de six mois pourrait être la vôtre pour les vacances de vos rêves ! Appelez-nous aujourd'hui au [color=#00FF00]RELAX-NOW[/color] pour réserver votre croisière. N'attendez pas ! Agissez maintenant !




                    {"[italic]*Les lacs de phosphore ne sont pas pour nager, vous renoncez à tous les droits aux représentations juridiques auprès de Relaxination Destinations lors de l'atterrissage sur Galimar.[/italic]"}
                    {"[italic]**L'œil du cosmos ne doit pas être regardé plus de cinq secondes à la fois. Vous n'entendez pas l'appel de l'œil.[/italic]"}
                    {"[italic]***Doit signer une décharge de responsabilité avant d'atterrir, Relaxination Destinations ne garantit pas la sécurité des ruines agathéiennes. Les disparitions de groupes de touristes ont diminué pour arriver à une marge acceptable de 0,23% de tous les groupes de touristes qui visitent les ruines.[/italic]"}

delivery-spam-angry-reader =  VOUS LES AVEZ TUÉS !!!!
.desc =         Une lettre à un éditeur.
.content =      À qui de droit,

                    La célèbre série de livres « Les aventures d'Ian et Renault » est celle qui est appréciée des jeunes enfants et d'au moins un lézard sévèrement dyslexique (ne vous inquiétez pas, je vais mieux). Cependant, il y avait une erreur dans la publication, mais Ian et Renault ne feraient jamais d'erreur en écrivant. Il semble que dans « Les aventures d'Ian et Renault : Un voyage arctique de courage et d'amitié », deux lignes de « Les aventures d'Ian et Renault : L'exploration de la ville » ont été utilisées.
                    Cela n'avait aucun sens. Comment Ian et Renault ont-ils trouvé un petit chaton dans l'arctique ? Que faisait le petit chaton là-bas ? Pourquoi Ian et Renault faisaient-ils plus que que d'habitude aujourd'hui ? Et la pire erreur est la deuxième ligne mal placée, qui est « Au fur et à mesure que le jour s'est transformé en nuit, ils étaient épuisés mais heureux de leur aventure en ville » (Ian et Renault, 1). OH, DÉSOLÉ !!! JE N'ÉTAIS PAS AU COURANT QU'IL Y AIT UNE VILLE DANS L'ARCTIQUE !!! QU'ILS SE SONT ARRÊTÉS. TU SAIS. POUR UN CAFÉ. OH ATTENDS !! LE CAFÉ TUE LES CHIENS ET LES RENARDS !!!!!!!!!! QUI PUBLIERAIT ÇA !!!!! TU VIENS DE TUER IAN ET RENAULT !!!!!!!!!!!!!!!!! COMMENT PEUX-TU !!!!!!!!!!!!!!!!!!! MONSTRE !!!!!!!!!!!!!!!!!
                    Et qu'as-tu reçu pour tuer Ian et Renault ? Hein ? Un livre un peu plus long ? Tu as besoin de ce livre plus long, n'est-ce pas ? Les livres plus longs se vendent plus cher, non ? Tu voulais faire plus d'argent ?
                    REGARDE OÙ ÇA T'A MENÉ. TU AS TUÉ LES DEUX. FÉLICITATIONS.
                    IL N'Y AURA PLUS DE SUITE MAINTENANT.
                    COMMENT TU VEUX FAIRE DE L'ARGENT MAINTENANT ???? C'EST NULS ????
                    {"[bold]BEN TU N'AURAIS PAS DÛ LES TUER !!!!!!!!!!!!!![/bold]"}
